...
void find_bucket(struct node buck,int l,int r,int low,int high,int pos)
{
	...
	find_bucket(buck,l,r,low,mid,2*pos);
	find_bucket(buck,l,r,mid+1,high,2*pos+1);/*
	int i=0;
	while(i<10)
	{
...
void update_tree(int i,int low,int high,int val,int pos)
{
	...
	//tree[pos].store[val]=tree[2*pos].store[val]+tree[2*pos+1].store[val];
	//tree[pos].store[s_array[i]]=tree[2*pos].store[s_array[i]]+tree[2*pos+1].store[s_array[i]];
